...
Supports TEST_MODE for instant email with mock data (no real API calls).
"""
import os
import smtplib
import logging
...
def send_email(subject: str, html_body: str, recipients: list) -> bool:
    if not SMTP_USER or not SMTP_PASSWORD:
        logger.error("Missing SMTP_USER or SMTP_PASSWORD")
        return False
...
def main():
    logger.info(f"Starting | session={SESSION} | test_mode={TEST_MODE}")
    logger.info(f"SMTP_USER: {'set' if SMTP_USER else 'MISSING'}")
    logger.info(f"SMTP_PASSWORD: {'set' if SMTP_PASSWORD else 'MISSING'}")
